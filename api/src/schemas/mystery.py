...
class SimilarMysteryNode(BaseModel):
    """Similar mystery with similarity score."""
    id: str = Field(description="Unique identifier for the similar mystery")
    title: str = Field(description="Title of the similar mystery")
    score: float = Field(description="Similarity score (0.3-1.0)")
    reasons: List[str] = Field(default_factory=list, description="Reasons for similarity")
...
class MysteryDetail(MysteryBase):
    ...
    similar_mysteries: List[SimilarMysteryNode] = Field(default_factory=list, description="Similar mysteries based on shared attributes")
